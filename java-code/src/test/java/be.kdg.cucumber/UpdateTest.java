...
public class UpdateTest {
    //normaal wouden wij deze 4 @Given ergens anders zetten maar dan kregen wij een error die wij na lang zoeken nog altijd niet opgelost kregen.
    @Given("Koeriers")
    ...
    public void erZijnBestellingenWaarvoorDeKoerierInAanmerkingKomt(int aantal) {
        assertEquals(aantal, DeliDish.leverOpdrachten.size());
    }

    @When("de koerier drukt op de refresh knop van de bestellingen")
    public void deKoerierDruktOpDeRefreshKnopVanDeBestellingen() {
        for (Bestelling bestelling : DeliDish.bestellingen) {
            bestelling.toString();
        }
    }

    @Then("toont het systeem de {int} bestellingen waarvan de status {string} is")
    public void toontHetSysteemDeBestellingenWaarvanDeStatusIs(int aantal, String status) {
        int teller = 0;
        for (Bestelling bestelling : DeliDish.bestellingen) {
            if (status.equals("OPEN")) teller++;
        }
        assertEquals(teller, DeliDish.bestellingen.size(), "Het aantal bestellingen moet gelijk zijn");
    }
}